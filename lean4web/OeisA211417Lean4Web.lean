...
/-!
# OEIS A211417: standalone Lean4Web targets

This mathlib-only file reproduces the sequence and the five substantive Formal Conjectures
targets.  The current vacuous statement `general_divisibility` is deliberately excluded.
-/

namespace OeisA211417Lean4Web

/-- The OEIS A211417 factorial-ratio sequence. -/
def a (n : ℕ) : ℕ :=
  (Nat.factorial (30 * n) * Nat.factorial n) /
    (Nat.factorial (15 * n) * Nat.factorial (10 * n) * Nat.factorial (6 * n))

/-- The local prime-power contribution to the valuation of `a(x)`. -/
...
/-- Every odd divisor `d ≥ 9` of `2n+1` contributes exactly one. -/
...
theorem seven_mul_a_dvd_two_mul_add_one_nat (n : ℕ) :
  ...
  have haeq : a n = factorialNumerator n / factorialDenominator n := rfl
  have hN0 : factorialNumerator n ≠ 0 := by
    exact Nat.mul_ne_zero (Nat.factorial_ne_zero _) (Nat.factorial_ne_zero _)
  have hD0 : factorialDenominator n ≠ 0 := by
    exact Nat.mul_ne_zero
...
/-- Exact standalone target: `(2n + 1) ∣ 7 a(n)`. -/
...
/-- Exact standalone target: `(3n + 1) ∣ a(n)`. -/
...
/-- Exact standalone target: `(5n + 1) ∣ a(n)`. -/
...
/-- The product target is a formal consequence of the three atomic targets. -/
...
These deliberately thin aliases collect the four completed exact targets in one place.  The
`#print axioms` commands immediately following them must not report `sorryAx`.
-/
